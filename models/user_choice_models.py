...
class TranslationContext:
    ...
    def calculate_similarity(self, other: TranslationContext) -> float:
        ...
        if self.semantic_field and other.semantic_field:
            similarity_factors.append(1.0 if self.semantic_field == other.semantic_field else 0.0)
        
        # Philosophical domain similarity
        if self.philosophical_domain and other.philosophical_domain:
            similarity_factors.append(1.0 if self.philosophical_domain == other.philosophical_domain else 0.0)
        ...
        lang_match = (self.source_language == other.source_language and 
        ...
        if self.related_concepts and other.related_concepts:
            ...
            concept_similarity = len(overlap) / len(total_concepts) if total_concepts else 0.0
    ...
    def add_choice_stats(self, choice: UserChoice) -> None:
        ...
        if self.total_choices > 0:
            self.average_confidence = (
                (self.average_confidence * (self.total_choices - 1) + choice.confidence_level) / 
                self.total_choices
            )
...
class ChoiceConflict:
    ...
    def analyze_conflict(self) -> None:
        ...
        self.context_similarity = self.choice_a.context.calculate_similarity(self.choice_b.context)
    ...
    def resolve_conflict(self, resolution_strategy: ConflictResolution) -> Optional[str]:
...
class TranslationPreference:
    ...
    def update_language_preference(self, source_lang: str, target_lang: str, preferences: Dict[str, Any]) -> None:
        """Update preferences for a specific language pair."""
        key = f"{source_lang}-{target_lang}"
        self.language_pair_preferences[key] = preferences
        self.updated_at = datetime.now().isoformat()
    
    def update_domain_preference(self, domain: str, preferences: Dict[str, Any]) -> None:
        """Update preferences for a specific domain."""
        self.domain_preferences[domain] = preferences
        self.updated_at = datetime.now().isoformat()
    
    def get_language_preference(self, source_lang: str, target_lang: str) -> Dict[str, Any]:
...
def filter_choices_by_context(choices: List[UserChoice], target_context: TranslationContext) -> List[UserChoice]:
    """Filter choices that are applicable to a given context."""
    return [choice for choice in choices if choice.is_applicable_to_context(target_context)]
