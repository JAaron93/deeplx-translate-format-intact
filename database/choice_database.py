...
from models.user_choice_models import (
    UserChoice, ChoiceSession, ChoiceConflict, TranslationPreference, TranslationContext,
...
class ChoiceDatabase:
    ...
    def __init__(self, db_path: str = "user_choices.db",
        ...
        logger.info(f"Configuration: alpha={self._learning_rate_alpha}, ensure_ascii={self._ensure_ascii}, batch_size={self._batch_size}")
    ...
    def save_user_choice(self, choice: UserChoice) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                """, (
                    choice.choice_id, choice.neologism_term, choice.choice_type.value,
                    choice.translation_result, choice.choice_scope.value,
                    choice.confidence_level, choice.user_notes, choice.created_at,
                    choice.updated_at, choice.last_used_at, choice.usage_count,
                    choice.success_rate, choice.session_id, choice.document_id,
                    choice.parent_choice_id, choice.is_validated,
                    choice.validation_source, choice.quality_score,
                    json.dumps(list(choice.export_tags)), choice.import_source,
    ...
    def _save_choice_context(self, conn: sqlite3.Connection, choice: UserChoice) -> None:
        ...
        """, (
            context_id, choice.choice_id, choice.context.generate_context_hash(),
            choice.context.semantic_field, choice.context.philosophical_domain,
            choice.context.author, choice.context.source_language,
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor = conn.execute("""
                    ...
                    LIMIT ?
                """, (
                    context.semantic_field, context.philosophical_domain,
                    context.author, context.source_language, context.target_language,
                    limit
                ))
        ...
        try:
            with self._get_connection() as conn:
                ...
                conn.execute("""
                    ...
                    WHERE choice_id = ?
                """, (
                    new_count, new_rate, datetime.now().isoformat(),
                    datetime.now().isoformat(), choice_id
                ))
    ...
    def delete_user_choice(self, choice_id: str) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                conn.execute("DELETE FROM choice_contexts WHERE choice_id = ?", (choice_id,))
                
                # Delete the choice
                cursor = conn.execute("DELETE FROM user_choices WHERE choice_id = ?", (choice_id,))
    ...
    def save_session(self, session: ChoiceSession) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                """, (
                    session.session_id, session.session_name, session.status.value,
                    session.document_id, session.document_name, session.user_id,
                    session.source_language, session.target_language,
                    session.created_at, session.updated_at, session.completed_at,
                    session.total_choices, session.translate_count,
                    session.preserve_count, session.custom_count, session.skip_count,
                    session.average_confidence, session.consistency_score,
                    session.auto_apply_choices, session.conflict_resolution_strategy.value,
                    session.session_notes, json.dumps(list(session.session_tags))
                ))
    ...
    def delete_session(self, session_id: str) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                conn.execute("DELETE FROM user_choices WHERE session_id = ?", (session_id,))
                
                # Delete conflicts
                conn.execute("DELETE FROM choice_conflicts WHERE session_id = ?", (session_id,))
                
                # Delete the session
                cursor = conn.execute("DELETE FROM choice_sessions WHERE session_id = ?", (session_id,))
    ...
    def save_conflict(self, conflict: ChoiceConflict) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                """, (
                    conflict.conflict_id, conflict.neologism_term,
                    conflict.choice_a.choice_id, conflict.choice_b.choice_id,
                    conflict.conflict_type, conflict.severity, conflict.context_similarity,
                    conflict.resolution_strategy.value, conflict.resolved_choice_id,
                    conflict.resolution_notes, conflict.detected_at, conflict.resolved_at,
                    conflict.session_id, conflict.auto_resolved
                ))
    ...
    def resolve_conflict(self, conflict_id: str, resolution_strategy: ConflictResolution, 
                        resolved_choice_id: Optional[str] = None, notes: str = "") -> bool:
        """Resolve a conflict."""
        try:
            with self._get_connection() as conn:
                now = datetime.now().isoformat()
                cursor = conn.execute("""
                    ...
                    WHERE conflict_id = ?
                """, (
                    resolution_strategy.value, resolved_choice_id, notes, now, conflict_id
                ))
    ...
    def _row_to_user_choice(self, row: sqlite3.Row) -> UserChoice:
        ...
        context = TranslationContext(
            ...
            context_similarity_threshold=context_data.get('context_similarity_threshold', 0.8),
    ...
    def _row_to_choice_session(self, row: sqlite3.Row) -> ChoiceSession:
        ...
        return ChoiceSession(
            ...
            conflict_resolution_strategy=ConflictResolution(row['conflict_resolution_strategy']),
    ...
    def get_database_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor = conn.execute("SELECT COUNT(*) FROM choice_sessions WHERE status = 'active'")
                ...
                cursor = conn.execute("SELECT COUNT(*) FROM choice_conflicts WHERE resolved_at IS NULL")
    ...
    def export_choices_to_json(self, session_id: Optional[str] = None, 
        ...
        try:
            ...
            if session_id:
                ...
                with self._get_connection() as conn:
                    ...
                    choices = [self._row_to_user_choice(row) for row in cursor.fetchall()]
            ...
            json_data = json.dumps(export_data, indent=2, ensure_ascii=self._ensure_ascii)
    ...
    def import_choices_from_json(self, json_data: str, session_id: Optional[str] = None) -> int:
        """
        ...
        try:
            ...
            for i, choice_data in enumerate(choices_data):
                try:
                    choice = self._validate_and_reconstruct_choice(choice_data, session_id)
            ...
            logger.info(f"Batch import completed: {imported_count}/{len(choices_data)} choices imported in {duration:.2f}s")
            logger.info(f"Performance: {imported_count/duration:.1f} choices/second")
            
            if validation_errors:
                logger.warning(f"Import completed with {len(validation_errors)} validation errors")
    ...
    def _validate_and_reconstruct_choice(self, choice_data: Dict[str, Any], session_id: Optional[str] = None) -> UserChoice:
        """
        ...
        if not isinstance(confidence_level, (int, float)) or not (0.0 <= confidence_level <= 1.0):
            raise ValueError(f"Invalid confidence_level: must be between 0.0 and 1.0, got {confidence_level}")
        
        usage_count = choice_data.get('usage_count', 0)
        if not isinstance(usage_count, int) or usage_count < 0:
            raise ValueError(f"Invalid usage_count: must be non-negative integer, got {usage_count}")
        
        success_rate = choice_data.get('success_rate', 1.0)
        if not isinstance(success_rate, (int, float)) or not (0.0 <= success_rate <= 1.0):
            raise ValueError(f"Invalid success_rate: must be between 0.0 and 1.0, got {success_rate}")
        
        quality_score = choice_data.get('quality_score', 0.0)
        if not isinstance(quality_score, (int, float)):
            raise ValueError(f"Invalid quality_score: must be numeric, got {quality_score}")
            ...
            with self._get_connection() as conn:
                ...
                for i in range(0, len(choices), self._batch_size):
                    ...
                    try:
                        ...
                        logger.error(f"Error importing batch {i//self._batch_size + 1}: {e}")
                    ...
                    # Log progress for large imports
                    if len(choices) > 1000:
                        logger.info(f"Imported batch {i//self._batch_size + 1}: {batch_imported}/{len(batch)} choices")
                
                conn.commit()
                if failed_count > 0:
                    logger.warning(f"Bulk import completed with errors: {imported_count} succeeded, {failed_count} failed")
                else:
                    logger.info(f"Bulk import transaction committed: {imported_count} choices")
                return imported_count
    ...
    def _import_choice_batch(self, conn: sqlite3.Connection, batch: List[UserChoice]) -> int:
        """
    ...
    def get_all_sessions(self) -> List[ChoiceSession]:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.execute("SELECT * FROM choice_sessions ORDER BY created_at")
                return [self._row_to_choice_session(row) for row in cursor.fetchall()]
        except Exception as e:
    ...
    def get_all_context_hashes(self) -> List[str]:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.execute("SELECT DISTINCT context_hash FROM choice_contexts")
