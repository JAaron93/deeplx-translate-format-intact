...
class PhilosophyTranslationExamples:
    ...
    def example_1_basic_text_translation(self):
        ...
        print(f"Translating from English to German...")
    ...
    def example_4_progress_tracking(self):
        ...
        def progress_callback(progress: PhilosophyTranslationProgress):
            print(f"Progress: {progress.overall_progress:.1f}% - Stage: {progress.current_stage}")
            ...
            print(f"  User Choice Application: {progress.user_choice_application_progress}%")
    ...
    async def example_5_document_processing(self):
        ...
        try:
            ...
            def progress_callback(progress: PhilosophyProcessingProgress):
                print(f"Document Processing: {progress.overall_progress:.1f}% - {progress.current_stage}")
                print(f"  Pages: {progress.processed_pages}/{progress.total_pages}")
                print(f"  Neologisms: {progress.processed_neologisms}/{progress.total_neologisms}")
            ...
            print(f"Total neologisms detected: {result.document_neologism_analysis.total_neologisms}")
...
class StatisticsAndPerformanceExamples:
    ...
    def example_7_statistics_monitoring(self):
        ...
        for i, text in enumerate(sample_texts):
            ...
            print(f"Translation {i+1}: {result.neologism_analysis.total_neologisms} neologisms")
        
        # Get translation service statistics
        translation_stats = self.service.get_statistics()
        print("\nTranslation Service Statistics:")
        print(f"  Philosophy-enhanced translations: {translation_stats['philosophy_enhanced_stats']['total_translations']}")
        print(f"  Total neologisms detected: {translation_stats['philosophy_enhanced_stats']['total_neologisms_detected']}")
        print(f"  Average detection time: {translation_stats['philosophy_enhanced_stats']['average_detection_time']:.3f}s")
        
        # Get document processor statistics
        processor_stats = self.processor.get_statistics()
        print("\nDocument Processor Statistics:")
        print(f"  Documents processed: {processor_stats['philosophy_enhanced_processor_stats']['documents_processed']}")
        print(f"  Total processing time: {processor_stats['philosophy_enhanced_processor_stats']['total_processing_time']:.2f}s")
