...
def demonstrate_basic_choice_workflow(manager, detector):
    ...
    for neologism in analysis.detected_neologisms:
        ...
        if neologism.term == "Dasein":
            ...
            print(f"✓ Made choice: TRANSLATE → 'being-there'")
            ...
            print(f"✓ Made choice: TRANSLATE → 'forgetfulness-of-being'")
            ...
            print(f"✓ Made choice: TRANSLATE → 'temporality'")
            ...
            print(f"✓ Made choice: PRESERVE original term")
    ...
    print(f"✓ Session completed with consistency score: {session.consistency_score:.2f}")
...
def demonstrate_conflict_resolution(manager, detector):
    ...
    if dasein_neologism:
        ...
        print(f"✓ Made conflicting choice: TRANSLATE → 'existence'")
        ...
        print(f"✓ Made conflicting choice: PRESERVE")
        ...
        for conflict in conflicts:
            ...
            print(f"    ✓ Resolved using highest confidence strategy")
...
def demonstrate_terminology_import_export(manager):
    ...
    reimported_count = manager.import_choices(import_data, new_import_session.session_id)
...
def demonstrate_advanced_features(manager, detector):
    ...
    for neologism in complex_analysis.detected_neologisms:
        ...
        if rec['suggested_action'] == 'apply_existing' and rec['existing_choice']:
            print(f"  → Applying existing choice: {rec['existing_choice']['translation']}")
...
def main():
    ...
    try:
        ...
        print(f"✓ Created {len([session1, session2, session3, session4, session5, session6])} sessions")
        print(f"✓ Processed multiple philosophical texts")
        print(f"✓ Demonstrated choice reuse and context matching")
        print(f"✓ Handled conflicts and resolutions")
        print(f"✓ Imported/exported terminology")
        print(f"✓ Showed advanced recommendation system")
        
        # Final statistics
        final_stats = manager.get_statistics()
        print(f"\nFinal Statistics:")
        print(f"  Total Choices Made: {final_stats['manager_stats']['total_choices_made']}")
        print(f"  Sessions Created: {final_stats['manager_stats']['sessions_created']}")
        print(f"  Conflicts Resolved: {final_stats['manager_stats']['conflicts_resolved']}")
        print(f"  Database Size: {final_stats['database_stats']['db_size_bytes']} bytes")
