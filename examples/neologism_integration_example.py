...
from models.neologism_models import NeologismAnalysis, DetectedNeologism, ConfidenceLevel
...
class PhilosophyEnhancedTranslator:
    ...
    def translate_with_neologism_detection(self, 
                                         text: str,
                                         source_lang: str = "de",
                                         target_lang: str = "en",
        ...
        """
        logger.info(f"Starting translation with neologism detection for {len(text)} characters")
        ...
        return {
            ...
            "processing_summary": {
                ...
                "high_confidence_count": len(neologism_analysis.get_high_confidence_neologisms()),
                "philosophical_density": neologism_analysis.philosophical_density_avg,
                "semantic_fields": neologism_analysis.semantic_fields
            }
        }
    
    def _prepare_text_for_translation(self, 
                                    text: str, 
                                    analysis: NeologismAnalysis) -> tuple[str, Dict[str, DetectedNeologism]]:
        """
    ...
    def _post_process_translation(self, 
                                translated_text: str, 
                                neologism_map: Dict[str, DetectedNeologism]) -> str:
        """
    ...
    def _generate_neologism_suggestions(self, 
    ...
    def _generate_morphological_breakdown(self, neologism: DetectedNeologism) -> Dict[str, Any]:
        ...
        if morphological.is_compound and morphological.compound_parts:
            breakdown["compound_interpretation"] = " + ".join(morphological.compound_parts)
...
def main():
    ...
    print(f"High confidence neologisms: {result['processing_summary']['high_confidence_count']}")
    print(f"Philosophical density: {result['processing_summary']['philosophical_density']:.3f}")
    print(f"Semantic fields: {', '.join(result['processing_summary']['semantic_fields'])}")
    ...
    for i, suggestion in enumerate(result['neologism_suggestions'], 1):
        ...
        if suggestion['morphological_breakdown']['is_compound']:
            print(f"   Compound parts: {', '.join(suggestion['morphological_breakdown']['compound_parts'])}")
