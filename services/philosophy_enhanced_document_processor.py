...
from .enhanced_document_processor import EnhancedDocumentProcessor, DocumentMetadata
from .philosophy_enhanced_translation_service import PhilosophyEnhancedTranslationService, PhilosophyTranslationProgress
from .neologism_detector import NeologismDetector
from .user_choice_manager import UserChoiceManager
from models.neologism_models import NeologismAnalysis, DetectedNeologism
from models.user_choice_models import ChoiceSession
...
class PhilosophyDocumentResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "page_neologism_analyses": [analysis.to_dict() for analysis in self.page_neologism_analyses],
...
class PhilosophyEnhancedDocumentProcessor:
    ...
    def __init__(self,
                 ...
                 philosophy_translation_service: Optional[PhilosophyEnhancedTranslationService] = None,
        ...
        self.philosophy_translation_service = philosophy_translation_service or PhilosophyEnhancedTranslationService(
    ...
    async def process_document_with_philosophy_awareness(self,
                                                       file_path: str,
                                                       source_lang: str,
                                                       target_lang: str,
                                                       provider: str = "auto",
                                                       ...
                                                       progress_callback: Optional[Callable[[PhilosophyProcessingProgress], None]] = None) -> PhilosophyDocumentResult:
        """
        ...
        try:
            ...
            original_content = await self._extract_content_async(file_path, progress, progress_callback)
            
            # Step 2: Create or get user session
            if not session_id:
                session_id = await self._create_session_async(file_path, user_id, source_lang, target_lang)
            ...
            translated_content = await self._translate_document_async(
                original_content, source_lang, target_lang, provider, session_id, progress, progress_callback
            )
            ...
            final_content = await self._reconstruct_document_async(
                translated_content, neologism_results, choice_results, progress, progress_callback
            )
            
            # Update statistics
            self.stats['documents_processed'] += 1
            self.stats['total_neologisms_detected'] += neologism_results['total_neologisms']
            self.stats['total_choices_applied'] += choice_results['total_choices_applied']
    ...
    async def _extract_content_async(self,
                                   file_path: str,
                                   progress: PhilosophyProcessingProgress,
                                   progress_callback: Optional[Callable[[PhilosophyProcessingProgress], None]] = None) -> Dict[str, Any]:
        """Extract content asynchronously with progress tracking."""
        # Run extraction in thread to avoid blocking
        content = await asyncio.to_thread(self.base_processor.extract_content, file_path)
        
        # Update progress metrics
        if content.get('type') == 'pdf_advanced' and 'layouts' in content:
            progress.total_pages = len(content['layouts'])
            progress.total_text_blocks = sum(len(layout.text_elements) for layout in content['layouts'])
    ...
    async def _detect_document_neologisms_async(self,
        ...
        if content.get('type') == 'pdf_advanced' and 'text_by_page' in content:
            for page_num, page_text in content['text_by_page'].items():
                if page_text.strip():
                    ...
                    progress.neologism_detection_progress = int(
                        (progress.processed_pages / progress.total_pages) * 100
                    ) if progress.total_pages > 0 else 100
    ...
    async def _apply_document_choices_async(self,
        ...
        for i, neologism in enumerate(document_analysis.detected_neologisms):
            ...
            existing_choice = await asyncio.to_thread(
                self.user_choice_manager.get_choice_for_neologism,
            ...
            progress.user_choice_progress = int(
                (progress.processed_neologisms / progress.total_neologisms) * 100
            ) if progress.total_neologisms > 0 else 100
    ...
    async def _translate_document_async(self,
                                      content: Dict[str, Any],
                                      source_lang: str,
                                      target_lang: str,
                                      provider: str,
        ...
        def translation_progress_callback(translation_progress: PhilosophyTranslationProgress):
        ...
        result = await self.philosophy_translation_service.translate_document_with_neologism_handling(
            content, source_lang, target_lang, provider, session_id, translation_progress_callback
        )
        
        return result['translated_content']
    
    async def _reconstruct_document_async(self,
                                        translated_content: Dict[str, Any],
                                        neologism_results: Dict[str, Any],
        ...
        translated_content['philosophy_enhanced_metadata'] = {
            ...
            'page_analyses': [analysis.to_dict() for analysis in neologism_results['page_analyses']],
    ...
    async def create_translated_document_with_philosophy_awareness(self,
                                                                 processing_result: PhilosophyDocumentResult,
        ...
        metadata_path = str(Path(output_path).with_suffix('')) + '_philosophy_metadata.json'
        try:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if self.stats['documents_processed'] > 0:
            ...
            base_stats['average_neologisms_per_document'] = (
                self.stats['total_neologisms_detected'] / self.stats['documents_processed']
            )
        
        if self.stats['total_neologisms_detected'] > 0:
            base_stats['choice_application_rate'] = (
                self.stats['total_choices_applied'] / self.stats['total_neologisms_detected']
            )
    ...
    async def cleanup_expired_sessions(self) -> int:
        ...
        return await asyncio.to_thread(self.user_choice_manager.cleanup_expired_sessions)
