...
from models.user_choice_models import UserChoice, ChoiceType, ChoiceScope
...
class PhilosophyTranslationProgress:
    ...
    def overall_progress(self) -> float:
        ...
        choice_progress = (self.processed_neologisms / max(1, self.total_neologisms)) if self.total_neologisms > 0 else 1.0
        translation_progress = self.translation_progress / 100.0
        
        return (detection_progress * detection_weight + 
                choice_progress * choice_weight + 
                translation_progress * translation_weight) * 100.0
...
class PhilosophyEnhancedTranslationService:
    ...
    async def translate_text_with_neologism_handling(self,
                                                   text: str,
                                                   source_lang: str,
                                                   target_lang: str,
                                                   provider: str = "auto",
                                                   session_id: Optional[str] = None,
                                                   progress_callback: Optional[Callable[[PhilosophyTranslationProgress], None]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Applying user choices to {len(neologism_analysis.detected_neologisms)} neologisms")
            ...
            logger.info(f"Translating modified text with {len(preservation_result.preservation_markers)} preserved terms")
            translated_text = await self._translate_with_preservation_async(
                preservation_result.modified_text, source_lang, target_lang, provider, progress, progress_callback
            )
            ...
            self.stats['neologisms_detected'] += len(neologism_analysis.detected_neologisms)
    ...
    async def translate_batch_with_neologism_handling(self,
                                                    texts: List[str],
                                                    source_lang: str,
                                                    target_lang: str,
                                                    provider: str = "auto",
                                                    session_id: Optional[str] = None,
                                                    progress_callback: Optional[Callable[[PhilosophyTranslationProgress], None]] = None) -> List[Dict[str, Any]]:
        """
    ...
    async def _detect_neologisms_async(self,
                                     text: str,
                                     progress: PhilosophyTranslationProgress,
                                     progress_callback: Optional[Callable[[PhilosophyTranslationProgress], None]] = None) -> NeologismAnalysis:
        ...
        high_confidence_neologisms = [
            n for n in analysis.detected_neologisms 
    ...
    async def _apply_user_choices_async(self,
                                      text: str,
                                      analysis: NeologismAnalysis,
                                      session_id: Optional[str],
                                      progress: PhilosophyTranslationProgress,
                                      progress_callback: Optional[Callable[[PhilosophyTranslationProgress], None]] = None) -> NeologismPreservationResult:
    ...
    def _apply_choice_to_text(self,
                             text: str,
                             neologism: DetectedNeologism,
                             choice: UserChoice) -> Tuple[str, Optional[str]]:
    ...
    def _replace_term_with_marker(self,
                                 text: str,
                                 neologism: DetectedNeologism,
                                 marker: str) -> str:
    ...
    async def _translate_with_preservation_async(self,
                                               text: str,
                                               source_lang: str,
                                               target_lang: str,
                                               provider: str,
                                               progress: PhilosophyTranslationProgress,
                                               progress_callback: Optional[Callable[[PhilosophyTranslationProgress], None]] = None) -> str:
    ...
    async def _restore_preserved_neologisms_async(self,
                                                translated_text: str,
    ...
    async def translate_document_with_neologism_handling(self,
        ...
        try:
            ...
            for i, block in enumerate(text_blocks):
                ...
                if block.get('text', '').strip():
                    ...
                    translated_block['neologism_analysis'] = result.get('neologism_analysis', {})
                    ...
                    for block_num, block in enumerate(page['text_blocks']):
                        ...
                        if key in block_lookup:
                            ...
                            if 'neologism_analysis' in translated_block:
                                block['neologism_analysis'] = translated_block['neologism_analysis']
    ...
    def update_configuration(self, **kwargs) -> None:
        ...
        if 'preserve_neologisms_by_default' in kwargs:
            self.preserve_neologisms_by_default = kwargs['preserve_neologisms_by_default']
        
        if 'neologism_confidence_threshold' in kwargs:
            self.neologism_confidence_threshold = kwargs['neologism_confidence_threshold']
