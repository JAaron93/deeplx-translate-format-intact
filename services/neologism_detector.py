...
class NeologismDetector:
    ...
    def __init__(self,
        ...
        Args:
            ...
            philosophical_threshold: Minimum philosophical density for detection
            morphological_analyzer: Optional MorphologicalAnalyzer instance (for dependency injection)
            philosophical_context_analyzer: Optional PhilosophicalContextAnalyzer instance
        ...
        logger.info(f"NeologismDetector initialized with lazy loading for model: {spacy_model}")
    ...
    def _initialize_spacy_model(self) -> Optional[Any]:
        ...
        try:
            ...
            logger.warning(f"spaCy model '{self.spacy_model}' not found, trying fallback")
            try:
                ...
                logger.warning("No German spaCy model available, using linguistic fallback")
                return None
    ...
    def _load_terminology(self) -> Dict[str, str]:
        ...
        try:
            klages_path = Path(__file__).parent.parent / "config" / "klages_terminology.json"
            if klages_path.exists():
                with open(klages_path, 'r', encoding='utf-8') as f:
    ...
    def _load_philosophical_indicators(self) -> Set[str]:
        ...
        # Only add if terminology is already loaded, don't trigger lazy loading here
    ...
    def _load_german_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "abstract_suffixes": [
                ...
                "ismus", "ität", "ation", "ismus", "logie", "sophie"
            ],
            "compound_patterns": [
    ...
    def analyze_text(self, 
                    text: str, 
                    text_id: str = "unknown",
                    chunk_size: int = 2000) -> NeologismAnalysis:
        """
        ...
        logger.info(f"Analyzed text '{text_id}': {analysis.total_detections} neologisms detected")
        return analysis
        ...
        if self.nlp:
            ...
            for token in doc:
                ...
                if token.pos_ in ["NOUN", "PROPN"] or self._is_compound_word(token.text):
                    ...
                    candidates.append(self.CandidateTerm(
                        term=token.text,
                        start_pos=token.idx,
    ...
    def _extract_candidates_regex(self, text: str) -> List[CandidateTerm]:
        ...
        compound_patterns = [
            r'\b[A-ZÄÖÜ][a-zäöüß]{5,}(?:[A-ZÄÖÜ][a-zäöüß]+)+\b',  # CapitalizedCompounds
            r'\b[a-zäöüß]+(?:s|n|es|en|er|e|ns|ts)[a-zäöüß]{4,}\b',  # linked compounds
            r'\b[a-zäöüß]+(?:heit|keit|ung|schaft|tum|nis|sal|ismus|ität|logie|sophie)\b'  # abstract suffixes
        ]
    ...
    def _analyze_philosophical_context(self,
                                     term: str,
                                     text: str,
                                     start_pos: int,
                                     end_pos: int) -> PhilosophicalContext:
        """Analyze philosophical context around a term."""
        return self.philosophical_context_analyzer.analyze_context(term, text, start_pos, end_pos)
    
    
    def _calculate_confidence_factors(self,
                                    term: str,
                                    morphological: MorphologicalAnalysis,
                                    philosophical: PhilosophicalContext) -> ConfidenceFactors:
        """Calculate individual confidence factors."""
        return self.confidence_scorer.calculate_confidence_factors(term, morphological, philosophical)
    
    
    def _classify_neologism_type(self, morphological: MorphologicalAnalysis) -> NeologismType:
        ...
        if any(suffix in morphological.suffixes for suffix in ["ismus", "logie", "sophie"]):
            return NeologismType.PHILOSOPHICAL_TERM
        
        # Check for technical indicators
        if any(suffix in morphological.suffixes for suffix in ["ation", "ität", "ismus"]):
    ...
    def _calculate_analysis_quality(self, analysis: NeologismAnalysis) -> float:
        ...
        # Quality factors
        confidence_avg = sum(n.confidence for n in analysis.detected_neologisms) / len(analysis.detected_neologisms)
        type_diversity = len(set(n.neologism_type for n in analysis.detected_neologisms))
        
        quality = (confidence_avg * 0.6) + (min(1.0, type_diversity / 5.0) * 0.4)
        
        return quality
    
    def _calculate_coverage_ratio(self, analysis: NeologismAnalysis, text: str) -> float:
        ...
        return min(1.0, analyzed_terms / max(1, total_words * 0.1))  # Expect ~10% potential neologisms
    
    def _extract_semantic_fields(self, analysis: NeologismAnalysis) -> List[str]:
        """Extract semantic fields from analysis."""
        return self.philosophical_context_analyzer.extract_semantic_fields(analysis.detected_neologisms)
    
    def _extract_dominant_concepts(self, analysis: NeologismAnalysis) -> List[str]:
        """Extract dominant concepts from analysis."""
        return self.philosophical_context_analyzer.extract_dominant_concepts(analysis.detected_neologisms)
...
def analyze_document_batch(detector: NeologismDetector, 
                          texts: List[str], 
                          text_ids: Optional[List[str]] = None) -> List[NeologismAnalysis]:
...
def merge_neologism_analyses(analyses: List[NeologismAnalysis]) -> NeologismAnalysis:
