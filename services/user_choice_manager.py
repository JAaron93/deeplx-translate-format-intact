...
from models.user_choice_models import (
    ...
    create_choice_id, create_session_id, create_conflict_id,
    filter_choices_by_context, find_best_matching_choice, detect_choice_conflicts
)
...
class UserChoiceManager:
    ...
    def create_session(self, 
                      ...
                      target_language: str = "en") -> ChoiceSession:
    ...
    def get_choice_for_neologism(self, 
                                neologism: DetectedNeologism,
                                session_id: Optional[str] = None) -> Optional[UserChoice]:
        """
        ...
        if existing_choices:
            ...
            if best_match:
                ...
                logger.debug(f"Found matching choice for '{neologism.term}': {best_match.choice_id}")
                return best_match
        ...
        if similar_choices:
            ...
            applicable_choices = filter_choices_by_context(similar_choices, target_context)
            
            if applicable_choices:
                best_match = max(applicable_choices, key=lambda c: c.success_rate)
                self.stats['context_matches'] += 1
                logger.debug(f"Found contextually similar choice for '{neologism.term}': {best_match.choice_id}")
                return best_match
    ...
    def resolve_conflict(self, 
        ...
        return self.db.resolve_conflict(conflict_id, resolution_strategy, resolved_choice_id, notes)
    
    # Context Management
    
    def _create_context_from_neologism(self, neologism: DetectedNeologism) -> TranslationContext:
        ...
        context = TranslationContext(
            ...
            author=neologism.philosophical_context.author_terminology[0] if neologism.philosophical_context.author_terminology else "",
    ...
    def find_similar_contexts(self, 
                             context: TranslationContext,
                             similarity_threshold: float = 0.8) -> List[UserChoice]:
    ...
    def process_neologism_batch(self, 
                               neologisms: List[DetectedNeologism],
                               session_id: Optional[str] = None,
                               auto_apply_similar: bool = True) -> List[Tuple[DetectedNeologism, Optional[UserChoice]]]:
        """
        ...
        for neologism in neologisms:
            ...
            if (auto_apply_similar and 
                suggested_choice and 
                suggested_choice.success_rate > 0.8 and
                suggested_choice.choice_scope in [ChoiceScope.GLOBAL, ChoiceScope.CONTEXTUAL]):
                ...
                new_choice = self.make_choice(
                    ...
                    confidence_level=suggested_choice.confidence_level * 0.9,  # Slightly lower confidence
    ...
    def apply_choices_to_analysis(self, 
        ...
        for neologism in analysis.detected_neologisms:
            ...
            if existing_choice:
                ...
                results['pending_choices'].append({
                    ...
                    'context': neologism.sentence_context[:100] + "..." if len(neologism.sentence_context) > 100 else neologism.sentence_context
                })
    ...
    def export_session_choices(self, session_id: str, file_path: Optional[str] = None) -> Optional[str]:
    ...
    def import_terminology_as_choices(self, 
                                    terminology_dict: Dict[str, str],
                                    session_id: Optional[str] = None,
                                    source_language: str = "de",
                                    target_language: str = "en") -> int:
        """
    ...
    def validate_data_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if orphaned_contexts:
                report['recommendations'].append(f"Found {len(orphaned_contexts)} orphaned contexts that should be cleaned up")
            ...
            if missing_references:
                report['recommendations'].append(f"Found {len(missing_references)} choices with invalid session references")
            ...
            if invalid_sessions:
                report['recommendations'].append(f"Found {len(invalid_sessions)} sessions with invalid states")
            ...
            if duplicate_choices:
                report['recommendations'].append(f"Found {len(duplicate_choices)} duplicate choices that should be merged")
            ...
            if conflicting_choices:
                report['recommendations'].append(f"Found {len(conflicting_choices)} conflicting choices requiring resolution")
            ...
            if expired_sessions:
                report['recommendations'].append(f"Found {len(expired_sessions)} expired sessions ready for cleanup")
            ...
            if report['total_issues'] == 0:
                report['recommendations'].append("✓ Data integrity check completed successfully - no issues found")
            else:
                report['recommendations'].append(f"⚠ Found {report['total_issues']} data integrity issues requiring attention")
                
            # Add performance recommendations
            if report['expired_sessions'] > 10:
                report['recommendations'].append("Consider running cleanup_expired_sessions() to improve performance")
            
            if report['duplicate_choices'] > 0:
                report['recommendations'].append("Run database optimization to merge duplicate choices")
            
            if report['conflicting_choices'] > 5:
                report['recommendations'].append("Review conflict resolution settings and resolve pending conflicts")
            
            logger.info(f"Data integrity validation completed: {report['total_issues']} issues found")
                ...
                if len(choices) > 1:
                    ...
                    for i, choice1 in enumerate(choices):
                        for choice2 in choices[i+1:]:
                            ...
                            similarity = choice1.context.calculate_similarity(choice2.context)
                            if (similarity > 0.7 and
                                choice1.choice_type == ChoiceType.TRANSLATE and
                                choice2.choice_type == ChoiceType.TRANSLATE and
                                choice1.translation_result != choice2.translation_result):
    ...
    def get_recommendation_for_neologism(self, 
                                       neologism: DetectedNeologism,
                                       session_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if existing_choice:
            ...
            if existing_choice.success_rate > 0.8:
                ...
                recommendation['reasons'].append(f"High success rate choice available ({existing_choice.success_rate:.1%})")
            else:
                recommendation['suggested_action'] = 'review_existing'
                recommendation['reasons'].append(f"Existing choice has moderate success rate ({existing_choice.success_rate:.1%})")
        ...
        if similar_choices:
            ...
            if not existing_choice:
                ...
                recommendation['reasons'].append(f"Found {len(similar_choices)} similar context choices")
        ...
        if neologism.confidence > 0.8:
            ...
            recommendation['reasons'].append("Low confidence detection - may not be a neologism")
...
def process_neologism_analysis(manager: UserChoiceManager,
                              analysis: NeologismAnalysis,
                              session_id: Optional[str] = None) -> Dict[str, Any]:
...
def create_session_for_document(manager: UserChoiceManager,
                               document_name: str,
                               user_id: Optional[str] = None,
                               source_lang: str = "de",
                               target_lang: str = "en") -> ChoiceSession:
