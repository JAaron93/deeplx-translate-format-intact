...
class ConfidenceScorer:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            german_morphological_patterns: German morphological patterns dictionary
        """
        self.philosophical_indicators = philosophical_indicators or set()
        self.german_morphological_patterns = german_morphological_patterns or self._load_default_patterns()
    ...
    def calculate_confidence_factors(self, 
                                   term: str, 
        ...
        factors.philosophical_indicators = min(1.0, len(philosophical.philosophical_keywords) / 5.0)
        factors.semantic_coherence = 0.7 if philosophical.semantic_field != "general" else 0.3
        
        # Frequency factors (simplified - would need corpus analysis for full implementation)
        factors.rarity_score = self._calculate_rarity_score(term)
        factors.frequency_deviation = 0.6  # TODO: Implement corpus-based frequency analysis
        ...
        # Linguistic factors
        factors.phonological_plausibility = self._calculate_phonological_plausibility(term)
        factors.syntactic_integration = 0.7  # TODO: Implement syntactic context analysis
    ...
    def _calculate_pattern_score(self, term: str, morphological: MorphologicalAnalysis) -> float:
    ...
    def adjust_confidence_threshold(self, 
        ...
        if philosophical_density > 0.5:
            adjusted -= 0.05  # Lower threshold for high philosophical density
        
        # Adjust based on author context
        if context_factors.get("author_context") in ["klages", "heidegger", "nietzsche"]:
            adjusted -= 0.05  # Lower threshold for known philosophical authors
    ...
    def update_philosophical_indicators(self, new_indicators: Set[str]) -> None:
        ...
        logger.info(f"Updated philosophical indicators with {len(new_indicators)} new terms")