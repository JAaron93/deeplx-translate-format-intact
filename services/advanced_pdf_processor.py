...
class AdvancedPDFProcessor:
    ...
    def extract_document_layout(self, pdf_path: str) -> List[PageLayout]:
        ...
        try:
            total_pages = len(doc)
            logger.info(f"Processing {total_pages} pages with advanced layout preservation")
            ...
            for page_num in range(total_pages):
                ...
                logger.info(f"Page {page_num + 1}: {len(layout.text_elements)} text elements extracted")
    ...
    def _extract_text_elements(self, page: fitz.Page, page_num: int) -> List[TextElement]:
        ...
        logger.debug(f"Extracted {len(text_elements)} text elements from page {page_num + 1}")
        return text_elements
    
    def create_translated_pdf(self, original_layouts: List[PageLayout], 
        ...
        try:
            for layout in original_layouts:
                self._create_translated_page(new_doc, layout, 
                                           translated_texts.get(layout.page_num, []))
                
                logger.info(f"Page {layout.page_num + 1} reconstructed with translation")
    ...
    def _create_translated_page(self, doc: fitz.Document, layout: PageLayout, 
    ...
    def _insert_text_element(self, page: fitz.Page, element: TextElement, text: str) -> None:
    ...
    def load_layout_backup(self, backup_path: str) -> List[PageLayout]:
        ...
        try:
            ...
            assets_dir = (backup_file.parent / assets_dir_name) if assets_dir_name else None

            layouts: List[PageLayout] = []
            for layout_data in backup_data["layouts"]:
                # Reconstruct text elements
                text_elements = [TextElement(**elem) for elem in layout_data["text_elements"]]
    ...
    def generate_preview(self, layouts: List[PageLayout], max_pages: int = 3) -> str:
        ...
        for i, layout in enumerate(layouts[:max_pages]):
            ...
            for elem in layout.text_elements[:10]:
                preview_lines.append(f"- {elem.text[:100]}{'...' if len(elem.text) > 100 else ''}")
            
            if len(layout.text_elements) > 10:
                preview_lines.append(f"... and {len(layout.text_elements) - 10} more elements")
    ...
    def get_text_for_translation(self, layouts: List[PageLayout]) -> Dict[int, List[str]]:
