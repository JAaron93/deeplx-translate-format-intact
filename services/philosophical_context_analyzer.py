...
class PhilosophicalContextAnalyzer:
    ...
    def _load_philosophical_indicators(self) -> Set[str]:
        ...
        config_file_path = Path(__file__).parent.parent / "config" / "philosophical_indicators.json"
        ...
        try:
            if config_file_path.exists():
                ...
                logger.info(f"Loaded {len(indicators)} philosophical indicators from config file")
            else:
                logger.warning(f"Config file not found at {config_file_path}, using hardcoded indicators")
                raise FileNotFoundError("Config file not found")
                
        except (FileNotFoundError, json.JSONDecodeError, IOError) as e:
            logger.warning(f"Could not load config file: {e}, falling back to hardcoded indicators")
    ...
    def analyze_context(self, 
        ...
        context.surrounding_terms = self._extract_surrounding_terms(context_window, term)
    ...
    def _extract_context_window(self, text: str, start_pos: int, end_pos: int, window_size: int = 200) -> str:
    ...
    def _calculate_philosophical_density(self, context: str) -> float:
        ...
        philosophical_word_count = sum(1 for word in words if word in self.philosophical_indicators)
    ...
    def _extract_surrounding_terms(self, context: str, target_term: str) -> List[str]:
        ...
        if self.nlp:
            ...
            for token in doc:
                if (token.text.lower() != target_term.lower() and
            ...
            for word in words:
                ...
                if (cleaned.lower() != target_term.lower() and
                    len(cleaned) > 3):
    ...
    def _extract_conceptual_clusters(self, context: str) -> List[str]:
        ...
        cluster_patterns = {
            ...
            "phenomenology": ["husserl", "heidegger", "phänomenologie", "intentionalität"],
            "existentialism": ["kierkegaard", "nietzsche", "existenz", "angst", "freedom"],
            "philosophy_of_mind": ["bewusstsein", "geist", "mental", "psyche", "kognition"],
    ...
    def extract_semantic_fields(self, detected_neologisms: List[Any]) -> List[str]:
        ...
        for neologism in detected_neologisms:
            if hasattr(neologism, 'philosophical_context') and neologism.philosophical_context.semantic_field:
