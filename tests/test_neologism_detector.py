...
class TestNeologismDetector:
    ...
    def sample_philosophical_text(self):
        ...
        return """
        Das Bewusstsein ist die fundamentale Wirklichkeitserfahrung des Menschen. 
        In der Phänomenologie Husserls wird die Intentionalität als Grundstruktur 
        des Bewusstseins analysiert. Die Lebensweltthematik zeigt sich als 
        Wirklichkeitsbewusstsein, das durch die Spontaneität der Erfahrung 
        konstituiert wird. Diese Bewusstseinsphilosophie untersucht die 
        Zeitlichkeitsstrukturen des Lebens.
        """
    ...
    def test_philosophical_keywords_extraction(self, detector):
        ...
        context = "Das Bewusstsein und die Wirklichkeit sind zentrale Begriffe der Philosophie."
    ...
    def test_neologism_type_classification(self, detector):
        ...
        assert detector._classify_neologism_type(compound_analysis) == NeologismType.COMPOUND
        ...
        assert detector._classify_neologism_type(derived_analysis) == NeologismType.DERIVED
        ...
        assert detector._classify_neologism_type(philosophical_analysis) == NeologismType.PHILOSOPHICAL_TERM
    ...
    def test_text_analysis_with_detections(self, detector):
        ...
        Das Wirklichkeitsbewusstsein ist ein zentraler Begriff der Bewusstseinsphilosophie.
        Die Lebensweltthematik zeigt sich in der Zeitlichkeitsstruktur des Daseins.
        """
        
        analysis = detector.analyze_text(text_with_neologisms, "neologism_test")
        
        # Should detect some neologisms
        assert analysis.total_detections >= 0  # May be zero if threshold is high
    ...
    def test_chunk_processing(self, detector):
        ...
        assert len(chunks) > 1
        assert all(len(chunk) <= 250 for chunk in chunks)  # Allow some overhead
    ...
    def test_context_window_extraction(self, detector):
        ...
        context = detector._extract_context_window(text, start_pos, end_pos, window_size=20)
...
class TestNeologismModels:
    ...
    def test_confidence_level_mapping(self):
        ...
        high_conf = DetectedNeologism(
            term="test", confidence=0.9, neologism_type=NeologismType.COMPOUND,
            start_pos=0, end_pos=4, sentence_context="test"
        )
        
        medium_conf = DetectedNeologism(
            term="test", confidence=0.7, neologism_type=NeologismType.COMPOUND,
            start_pos=0, end_pos=4, sentence_context="test"
        )
        
        low_conf = DetectedNeologism(
            term="test", confidence=0.5, neologism_type=NeologismType.COMPOUND,
            start_pos=0, end_pos=4, sentence_context="test"
        )
        
        uncertain_conf = DetectedNeologism(
            term="test", confidence=0.3, neologism_type=NeologismType.COMPOUND,
            start_pos=0, end_pos=4, sentence_context="test"
        )
    ...
    def test_neologism_analysis_creation(self):
        ...
        neologism1 = DetectedNeologism(
            term="Test1", confidence=0.8, neologism_type=NeologismType.COMPOUND,
            start_pos=0, end_pos=5, sentence_context="test"
        )
        
        neologism2 = DetectedNeologism(
            term="Test2", confidence=0.6, neologism_type=NeologismType.DERIVED,
            start_pos=6, end_pos=11, sentence_context="test"
        )
    ...
    def test_confidence_factors_calculation(self):
        ...
        assert 0.0 <= weighted_score <= 1.0
        assert weighted_score > 0.5  # Should be relatively high given the input values
...
class TestErrorHandling:
    ...
    def test_empty_text_analysis(self, detector):
        ...
        assert analysis.total_tokens == 0
        assert analysis.analyzed_chunks == 1  # Empty text still counts as one chunk
...
class TestPerformance:
    ...
    def test_large_text_processing(self, detector):
        ...
        large_text = ("Das Bewusstsein ist ein wichtiger Begriff in der Philosophie. " * 1000)
        ...
        # Should complete in reasonable time (less than 30 seconds for this test)
    ...
    def test_memory_efficiency(self, detector):
        ...
        texts = [
            f"Text {i} with Bewusstsein and Wirklichkeit terms." 
            for i in range(100)
        ]
