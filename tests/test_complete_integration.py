...
def get_memory_limit_mb():
    ...
    return int(os.environ.get('TEST_MEMORY_LIMIT_MB', '1000'))  # Default 1GB instead of 500MB
...
class TestCompletePhilosophyEnhancedIntegration:
    ...
    def test_2_neologism_detection_integration(self, sample_philosophical_text):
        ...
        expected_terms = {"Dasein", "Sein-zum-Tode", "Angst", "Zuhandenheit", "Zeitlichkeit"}
        
        # At least some expected terms should be detected
        assert len(detected_terms & expected_terms) > 0
        
        logger.info(f"✓ Detected {analysis.total_neologisms} neologisms: {detected_terms}")
    ...
    def test_4_text_translation_with_neologism_handling(self, sample_philosophical_text):
        ...
        assert result.translated_text != sample_philosophical_text  # Should be different
        assert result.neologism_analysis is not None
        assert result.neologism_analysis.total_neologisms > 0
        
        logger.info(f"✓ Translation completed with {result.neologism_analysis.total_neologisms} neologisms detected")
    ...
    def test_5_batch_translation_integration(self):
        ...
        logger.info(f"✓ Batch translation completed: {len(results)} texts, {total_neologisms} total neologisms")
    ...
    async def test_6_document_processing_integration(self, temp_document_file):
        ...
        def progress_callback(progress: PhilosophyProcessingProgress):
            ...
            logger.info(f"Progress: {progress.overall_progress:.1f}% - {progress.current_stage}")
        ...
        logger.info(f"Neologisms detected: {result.document_neologism_analysis.total_neologisms}")
    ...
    async def test_8_convenience_function_integration(self, temp_document_file):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger.info(f"✓ Convenience function completed successfully")
    ...
    def test_9_statistics_and_monitoring(self, sample_philosophical_text):
        ...
        logger.info(f"✓ Statistics collection working")
        logger.info(f"Translation stats: {translation_stats['philosophy_enhanced_stats']}")
        logger.info(f"Processor stats: {processor_stats['philosophy_enhanced_processor_stats']}")
    ...
    def test_11_memory_and_performance_monitoring(self, sample_philosophical_text):
        ...
        logger.info(f"Average time per translation: {processing_time/len(results):.3f}s")
        ...
        assert memory_increase < memory_limit_mb, f"Memory increase {memory_increase:.1f}MB exceeded limit {memory_limit_mb}MB"
    ...
    def test_12_complete_end_to_end_workflow(self, temp_document_file):
        ...
        logger.info(f"Neologisms detected: {result.neologism_analysis.total_neologisms}")
...
async def test_async_complete_integration():
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as f:
        f.write("Dasein is fundamental to Heideggerian philosophy. Angst reveals groundlessness.")
        temp_path = f.name
