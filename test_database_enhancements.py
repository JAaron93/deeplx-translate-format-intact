...
Tests alpha parameter configuration, JSON encoding, and batch import optimization.
"""
...
from models.user_choice_models import UserChoice, ChoiceType, ChoiceScope, TranslationContext
...
def test_alpha_parameter_configuration():
    ...
    assert db.learning_rate_alpha == 0.1, f"Expected default alpha 0.1, got {db.learning_rate_alpha}"
    print("✓ Default alpha value (0.1) works")
    
    # Test 2: Custom alpha value
    db = ChoiceDatabase(":memory:", learning_rate_alpha=0.05)
    assert db.learning_rate_alpha == 0.05, f"Expected alpha 0.05, got {db.learning_rate_alpha}"
    print("✓ Custom alpha value (0.05) works")
    
    # Test 3: Alpha property setter
    db.learning_rate_alpha = 0.2
    assert db.learning_rate_alpha == 0.2, f"Expected alpha 0.2, got {db.learning_rate_alpha}"
    print("✓ Alpha property setter works")
    
    # Test 4: Alpha validation - invalid range
    try:
        ChoiceDatabase(":memory:", learning_rate_alpha=2.0)
        assert False, "Should have raised ValueError for learning_rate_alpha parameter with invalid value 2.0 (exceeds maximum of 1.0)"
    ...
    try:
        ...
        assert False, "Should have raised ValueError for learning_rate_alpha parameter with invalid value 0.0005 (below minimum of 0.001)"
    except ValueError as e:
        assert "between 0.001 and 1.0" in str(e)
    ...
    try:
        ...
        assert False, "Should have raised ValueError for learning_rate_alpha parameter with invalid type 'str' (value: '0.1')"
    except ValueError as e:
...
def test_json_encoding_configuration():
    ...
    db = ChoiceDatabase(":memory:")
    assert db.ensure_ascii == False, f"Expected default ensure_ascii False, got {db.ensure_ascii}"
    ...
    assert db.ensure_ascii == True, f"Expected ensure_ascii True, got {db.ensure_ascii}"
    print("✓ Custom ensure_ascii (True) works")
    
    # Test 3: ensure_ascii property setter
    db.ensure_ascii = False
    assert db.ensure_ascii == False, f"Expected ensure_ascii False, got {db.ensure_ascii}"
    ...
    with tempfile.NamedTemporaryFile(suffix='.db', delete=False) as tmp:
        ...
        if save_result:
            ...
            if json_data and "Bewußtsein" in json_data and "Phänomenologie" in json_data:
                print("✓ JSON export preserves international characters (ensure_ascii=False)")
            else:
    ...
    with tempfile.NamedTemporaryFile(suffix='.db', delete=False) as tmp:
        ...
        if save_result:
            ...
            if json_data and "\\u" in json_data:
                print("✓ JSON export escapes international characters (ensure_ascii=True)")
            else:
...
def test_batch_import_optimization():
    ...
    with tempfile.NamedTemporaryFile(suffix='.db', delete=False) as tmp:
        ...
        from models.user_choice_models import ChoiceSession, SessionStatus, ConflictResolution
        ...
        imported_count = db.import_choices_from_json(json_data, session_id="test_batch_session")
        ...
        assert stats['total_choices'] == 1000, f"Expected 1000 total choices, got {stats['total_choices']}"
        ...
        assert choice.neologism_term == "TestTerm500", f"Expected TestTerm500, got {choice.neologism_term}"
        assert choice.confidence_level == 0.8, f"Expected confidence 0.8, got {choice.confidence_level}"
        ...
        assert imported_count == 0, f"Expected 0 imported from invalid data, got {imported_count}"
...
def test_backward_compatibility():
    ...
    with tempfile.NamedTemporaryFile(suffix='.db', delete=False) as tmp:
        ...
        assert result == True, "Should be able to save choice"
